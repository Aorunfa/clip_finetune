...
def test(args, model, val_loader):
    # metric: recall and avg smilarity 
    # val_loader for no shuffle
    device = args.device
    model.eval()
    metric = {'recall': 0, 'smilarity': 0}
    for i, (images, texts) in enumerate(val_loader):
        ...
        recall = (preds == labels).sum() / len(labels)
        avg_smilarity = probs[labels, labels].mean()
        metric['recall'] = (metric['recall'] * i + recall) / (i + 1)
...
if __name__ == '__main__':
    ...
    增加checkpoint存储， 记录step + 时间 -- done
    增加csv表个记录val：train loss， lr, val loss, val metric  -- done
    """